...
import { resolveAmrProfile } from './vela-profile-stub.js';
import {
  buildAuthDiagnostic,
...
function withRememberedAmrModels(
  ...
  if (!rememberedModels || rememberedModels.length === 0) return modelResult;
...
async function probe(
  ...
  const authDiagnostic = auth ? buildAuthDiagnostic(def, auth) : undefined;
  return {
    ...stripFns(def),
