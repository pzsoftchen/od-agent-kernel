...
export function resolveMmdRouteLaunchEnv(
  ...
  return { ANTHROPIC_BASE_URL: baseUrl, ...(apiKey ? { ANTHROPIC_AUTH_TOKEN: apiKey } : {}) };
}

function addModel(out: RuntimeModelOption[], seen: Set<string>, option: RuntimeModelOption): void {
...
export function mergeMmdRouteModels(
  ...
  for (const routeId of routeIds) addModel(out, seen, { id: routeId, label: routeId });
...
export async function loadMmdRouteModels(
  ...
  let text: string;
  try { text = await readFile(routesFile, 'utf8'); } catch { return null; }
  let parsed: unknown;
