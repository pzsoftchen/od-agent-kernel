---
name: contract-review
description: Legal contract review and risk analysis
triggers: [contract, legal review, legal audit, agreement, NDA, terms of service]
---

# Contract Review Workflow

1. Identify the contract type and governing law
2. Review each clause against the legal framework
3. Flag deviations from standard terms with risk ratings
4. Check for missing essential clauses
5. Generate a redlines summary with recommended changes
6. Provide an overall risk assessment
